...
class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        mContext = context;
        isTimeTextBold = ta.getBoolean(R.styleable
                .CountdownView_isTimeTextBold, false);
        mTimeTextSize = ta.getDimension(R.styleable
        ...
        mSuffixMinuteLeftMargin = ta.getDimension(R.styleable
                .CountdownView_suffixMinuteLeftMargin, -1);
        mSuffixMinuteRightMargin = ta.getDimension(R.styleable
                .CountdownView_suffixMinuteRightMargin, -1);
        mSuffixSecondLeftMargin = ta.getDimension(R.styleable
                .CountdownView_suffixSecondLeftMargin, -1);
        mSuffixSecondRightMargin = ta.getDimension(R.styleable
                .CountdownView_suffixSecondRightMargin, -1);
        mSuffixMillisecondLeftMargin = ta.getDimension(R.styleable
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    private void initTimeTextBaseline(int viewHeight, int viewPaddingTop, int viewPaddingBottom) {
        if (viewPaddingTop == viewPaddingBottom) {
            ...
            // padding top
            mTimeTextBaseline = viewHeight - (viewHeight - viewPaddingTop) + mTimeTextHeight -
                    mTimeTextBottom;
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(Utils.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2,
            ...
            // hour left point
            mHourLeft = mLeftPaddingSize + mDayTimeTextWidth + mSuffixDayTextWidth +
        ...
        if (isShowHour) {
            ...
            canvas.drawText(Utils.formatNum(mHour), mHourLeft + curTimeTextWidth / 2,
                    mTimeTextBaseline, mTimeTextPaint);
            ...
            // minute left point
            mMinuteLeft = mHourLeft + curTimeTextWidth + mSuffixHourTextWidth +
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(Utils.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2,
                    mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowSecond) {
            // draw second text
            canvas.drawText(Utils.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2,
                    mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(Utils.formatMillisecond(mMillisecond), mMillisecondLeft +
