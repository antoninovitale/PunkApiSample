...
public class ExpandableTextView extends AppCompatTextView {
  ...
  final OnGlobalLayoutListener onGlobalLayoutListener = new OnGlobalLayoutListener() {
    ...
    @SuppressWarnings("deprecation") @Override public void onGlobalLayout() {
      ...
      if (!mCollapsed && mMaxLines > 0 && getLineCount() > mMaxLines) {
        ...
        String text = String.format("%s%s %s", getText().subSequence(0,
            lineEndIndex - mViewMoreText.length() - mEllipsizeText.length() + 1), mEllipsizeText,
            mViewMoreText);
        ...
        String text = String.format("%s %s", getText().subSequence(0, lineEndIndex), mViewLessText);
        setMovementMethod(LinkMovementMethod.getInstance());
  ...
  public ExpandableTextView(Context context, AttributeSet attrs) {
    ...
    try {
      mMaxLines = a.getInt(R.styleable.ExpandableTextView_maxLinesToCollapse, DEFAULT_MAX_LINES);
  ...
  private SpannableStringBuilder addClickablePartTextViewResizable(final String strSpanned,
    ...
    if (strSpanned.contains(spanableText)) {
      ssb.setSpan(new ClickableSpan() {
                  ...
                  }, strSpanned.indexOf(spanableText), strSpanned.indexOf(spanableText) + spanableText.length(),
          0);
    }
    return ssb;
  }
